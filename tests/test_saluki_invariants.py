...
from saluki_plugin.dataset import RNACNNDataset, SALUKI_BLOCKSIZE
...
class MockTokenizer:
    def __init__(self):
        ...
        self.backend_tokenizer.pre_tokenizer.pre_tokenize_str.return_value = [("ACGT", (0, 4))]
...
def test_saluki_enforces_blocksize_invariant(mock_args):
    ...
    assert tokenizer.model_max_length == SALUKI_BLOCKSIZE, \
...
def test_saluki_rejects_non_atomic_encoding(mock_args):
    ...
    with pytest.raises(ValueError, match="Saluki requires tokenization.encoding='atomic'"):
        with patch("builtins.open", unittest.mock.mock_open(read_data="id1\t1.0\tACGT\n")):
            RNACNNDataset(
                tokenizer=tokenizer,
                args=mock_args,
